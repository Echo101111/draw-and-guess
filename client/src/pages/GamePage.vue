  ...
  <div class="game-page">
    ...
    <main class="game-content">
      ...
      <div class="game-area">
        ...
        <Transition name="fade">
          <div v-if="showWordConfigGameover" class="word-config-overlay" @click.self="showWordConfigGameover = false">
            <div class="word-config-modal">
              <div class="word-config-modal-header">
                <span>⚙️ 词库设置</span>
                <button class="word-config-modal-close" @click="showWordConfigGameover = false">✕</button>
              </div>
              <div class="word-config-modal-body">
                <div class="word-config-field">
                  ...
                  <div class="custom-words-list">
                    <div v-for="(item, index) in gameoverWords" :key="index" class="custom-word-row">
                      ...
                      <button class="btn-remove-word" @click="gameoverRemoveWord(index)" :disabled="gameoverWords.length <= 1">✕</button>
                    </div>
                  </div>
                  <button type="button" class="btn-add-word" @click="gameoverAddWord">+ 添加词汇</button>
                  <div v-if="gameoverWords.filter(w => w.word.trim()).length > 0" class="word-count-hint">
                    共 {{ gameoverWords.filter(w => w.word.trim()).length }} 个词汇 → {{ gameoverWords.filter(w => w.word.trim()).length }} 轮
                  </div>
                </div>
                <div class="word-config-field">
                  <label class="checkbox-label">
                    <input type="checkbox" v-model="gameoverLooseMatching" @change="gameoverSaveConfig" />
                    宽松匹配（接受同义词/近似答案）
                  </label>
                </div>
                <button class="btn-save-custom" @click="gameoverSaveConfig">保存配置</button>
              </div>
            </div>
          </div>
        </Transition>
...
const gameoverLooseMatching = ref(false)
...
function gameoverRemoveWord(index: number) {
  gameoverWords.value.splice(index, 1)
}

function gameoverSaveConfig() {
  const customWords = gameoverWords.value
...
watch(() => roomStore.room?.wordConfig, (wc) => {
  if (wc && wc.customWords.length > 0 && gameoverWords.value.length <= 1 && !gameoverWords.value[0]?.word) {
    gameoverWords.value = wc.customWords.map(w => ({
      word: w.word,
